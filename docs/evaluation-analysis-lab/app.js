...
function renderIOC(){
 ...
 '<td><div class="signed-input-wrap"><input aria-label="sum '+r.item+'" data-sum="'+i+'" type="text" inputmode="numeric" pattern="-?[0-9]*" value="'+(s["sum"+i]??"")+'">'+(r.sum<0?'<button type="button" class="sign-toggle" data-sign-type="sum" data-sign-index="'+i+'">ใส่ −</button>':'')+'</div></td>'+
 '<td><div class="signed-input-wrap"><input aria-label="ioc '+r.item+'" data-ioc="'+i+'" type="text" inputmode="decimal" pattern="-?[0-9]*[.,]?[0-9]*" value="'+(s["ioc"+i]??"")+'">'+(r.ioc<0?'<button type="button" class="sign-toggle" data-sign-type="ioc" data-sign-index="'+i+'">ใส่ −</button>':'')+'</div></td>'+
 ...
 document.querySelectorAll('[data-sign-type]').forEach(btn=>btn.onclick=()=>{
   const i=btn.getAttribute("data-sign-index");
   const type=btn.getAttribute("data-sign-type");
   const el=document.querySelector(type==="sum"?'[data-sum="'+i+'"]':'[data-ioc="'+i+'"]');
   let v=(el.value||"").trim();
   if(!v){el.value="-";}
   else if(v.startsWith("-")){el.value=v.slice(1);}
   else{el.value="-"+v;}
   btn.textContent=el.value.startsWith("-")?"เอา − ออก":"ใส่ −";
   el.focus();
   saveIOC();
 });
